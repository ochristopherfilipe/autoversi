...
# Função para carregar a estrutura da Bíblia (capítulos e versículos) a partir do CSV
import csv

def carregar_estrutura_biblia(caminho_csv="capitulos-versiculos.csv"):
    estrutura = {}
    with open(caminho_csv, "r", encoding="utf-8-sig") as f:
        # Ajustar o delimitador caso seu CSV use outro separador
        reader = csv.DictReader(f, delimiter=',')
        
        # Imprime as colunas encontradas
        print("Colunas encontradas no CSV:", reader.fieldnames)
        
        # Remover espaços das colunas se houver
        fieldnames = [fn.strip() for fn in reader.fieldnames]
        
        # Verificar se 'Livro', 'Capítulo', 'Versículos' estão presentes
        if "Livro" not in fieldnames or "Capítulo" not in fieldnames or "Versículos" not in fieldnames:
            raise ValueError("O CSV não contém as colunas esperadas: 'Livro', 'Capítulo', 'Versículos'.")

        # Recriar o leitor com as fieldnames ajustadas
        reader = csv.DictReader(f, delimiter=',', fieldnames=fieldnames)
        next(reader)  # pular a linha do cabeçalho manualmente já que reiniciamos o reader

        for row in reader:
            # Remover espaços das strings lidas
            livro = row["Livro"].strip()
            cap = row["Capítulo"].strip()
            vers = row["Versículos"].strip()
            
            if not livro or not cap or not vers:
                continue
...
estrutura_biblia = carregar_estrutura_biblia("capitulos-versiculos.csv")
        ...
        if capitulo.isdigit() and len(capitulo) > 1 and versiculo == '1':
            ...
            for i in range(1, len(capitulo)):
                ...
                if val:
                    ...
                    break
        if not valido:
            return {'sucesso': False, 'mensagem': f"Erro: Não foi possível corrigir a referência {livro_normalizado} {capitulo}:{versiculo}."}
...
class App:
    def __init__(self, master):
        ...
        self.slider_tempo_captura.set(5)
        ...
        self.palavras_proximo = [
            "próximo", "próximo versículo", "continuando", "mais um", "seguindo",
            "depois desse", "o próximo", "ir para o próximo", "verso seguinte",
            "continuar", "prossiga", "segue", "pule para o próximo", "próximo texto",
            "continua", "vai para o próximo", "mais um versículo", "leia o próximo",
            "avançar", "passar para o próximo"
        ]

        self.palavras_anterior = [
            "anterior", "versículo anterior", "antes", "o versículo antes desse",
            "voltar", "voltar atrás", "retroceder", "o anterior", "volte",
            "verso anterior", "ir para o anterior", "leia o anterior", "pule para o anterior",
            "retorne", "retornar", "volte para o anterior", "recuar", "rever o anterior",
        ...
        try:
            ...
            for janela in janelas:
                if "Holyrics" in janela:
                    ...
                    time.sleep(1)
                    return
            self.label_status['text'] = "Janela do Holyrics não encontrada."
        except Exception as e:
    ...
    def iniciar_captura(self):
        if not self.capturando:
            ...
            if selected_mic == "Selecione o microfone":
                self.label_status['text'] = "Por favor, selecione um microfone."
                return
            try:
    ...
    def processar_boletim(self):
        ...
        if caminho_pdf:
            ...
            self.label_status['text'] = "Referências do boletim salvas."
        ...
        def callback(recognizer, audio):
            def processa_audio():
                try:
                    ...
                    self.label_status['text'] = "Desculpe, não entendi."
                except sr.RequestError as e:
                    self.label_status['text'] = f"Erro no serviço de voz: {e}"
